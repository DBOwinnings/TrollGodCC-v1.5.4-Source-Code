...
public final class EntityUtil implements Minecraftable
{
    ...
    public static boolean isntValid(final EntityPlayer entity, final double range) {
        return EntityUtil.mc.player.getDistance((Entity)entity) > range || entity == EntityUtil.mc.player || entity.getHealth() <= 0.0f || entity.isDead || TrollGod.INSTANCE.getFriendManager().isFriend(entity);
    }
    
    public static boolean isntValidFriend(final EntityPlayer entity, final double range) {
        return EntityUtil.mc.player.getDistance((Entity)entity) > range || entity == EntityUtil.mc.player || entity.getHealth() <= 0.0f || entity.isDead || !TrollGod.INSTANCE.getFriendManager().isFriend(entity);
    }
